...
exports.seed = async function (knex) {
  ...
  for (const order of orders) {
    ...
    for (const product of selectedProducts) {
      ...
      if (promoMapping && activePromos.length > 0) {
  ...
  for (const order of orders) {
    const orderRecord = await knex("orders").where("id", order.id).first();
    ...
    await knex("orders")
      .where("id", order.id)
      .update({ total_amount: itemsTotal + Number(orderRecord.shipping_fee) });
  }
};
