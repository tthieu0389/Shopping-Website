        ...
        <div className="max-w-[1200px] mx-auto flex items-center gap-5 h-16">
            ...
            {searchOpen && (query || loading) && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-shade rounded-xl shadow-lg z-50 overflow-hidden">
                ...
                {results.map(p => (
                  ...
                  >
                    <img src={p.img} alt={p.name} className="w-10 h-10 object-contain rounded-md bg-cream flex-shrink-0"/>
                    <div>
                      <div className="text-sm font-semibold text-body line-clamp-1">{p.name}</div>
                      <div className="text-xs text-accent font-bold">{p.price?.toLocaleString('vi-VN')}₫</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="flex items-center gap-2.5 ml-auto">
            {isAuthenticated ? (
              <div className="relative group">
                ...
                <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-shade rounded-xl shadow-lg z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                  ...
                  <button onClick={handleLogout} className="w-full flex items-center gap-2 px-4 py-3 text-sm hover:bg-cream rounded-b-xl text-accent">🚪 Đăng xuất</button>
                </div>
              </div>
            ) : (
              <>
                <Link to="/login" className="px-4 py-1.5 border border-vnpt text-vnpt rounded-full text-sm font-semibold hover:bg-vnpt hover:text-white transition-all">
                  Đăng nhập
                </Link>
                <Link to="/register" className="px-4 py-1.5 bg-vnpt text-white rounded-full text-sm font-semibold hover:bg-vnpt-dark transition-all">
                  Đăng ký
                </Link>
              </>
            )}
            ...
            <Link to="/cart" className="relative w-9 h-9 border border-shade rounded-lg flex items-center justify-center hover:border-vnpt hover:text-vnpt transition-all text-body">
              🛒
              {cartCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4.5 h-4.5 bg-accent text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-white min-w-[18px] px-0.5">
