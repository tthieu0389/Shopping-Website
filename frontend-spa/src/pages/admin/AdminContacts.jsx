...
import { Card, Btn, Modal, SearchInput, SelectPill } from './ui.jsx'
...
export default function AdminContacts() {
  ...
  const load = () => {
    ...
    contactApi.getAll()
      ...
      .catch(err => toast.error(translateApiError(err, 'Tải danh sách liên hệ thất bại')))
...
Đội ngũ hỗ trợ VNPT Shop`
  ...
  // chấm đỏ thông báo ở Navbar. Đây là cách phản hồi chính thức, khác với
  // nút "Mở Gmail" bên dưới (chỉ là tiện ích soạn email thủ công, không
  // lưu gì vào hệ thống).
  ...
  const confirmDelete = () => {
    ...
    setDeleting(true)
    contactApi.remove(deleteTarget.id)
      .then(() => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Popup xác nhận xoá */}
      {deleteTarget && (
        <Modal title="Xoá tin nhắn" onClose={() => setDeleteTarget(null)} width="max-w-[420px]">
          <p className="text-sm text-body mb-6">
            Bạn có chắc muốn xoá tin nhắn từ <strong>"{deleteTarget.name}"</strong>? Hành động này không thể hoàn tác.
          </p>
          <div className="flex justify-end gap-2.5">
            <Btn variant="ghost" onClick={() => setDeleteTarget(null)}>Huỷ</Btn>
